...
class BgFetcher(private val bus: Bus) {
    ...
    /**
     * Switches to the window map mid-line: the same fetcher pointed at LCDC.6, Y from the window's
     * own counter, X from 0 (no SCX/SCY). The caller clears the BG FIFO first; the refill here is
     * the window trigger's cost. No dummy fetch on the restart.
    ...
    private fun completeStep() {
        when (step) {
            Step.TILE_NUMBER -> {
                ...
                if (window) {
                    // Window: own Y counter, column from 0 (no SCX/SCY), LCDC.6 selects the map.
                    fineY = windowLine and 7
